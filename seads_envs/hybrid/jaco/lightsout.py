...
from seads_envs.hybrid.jaco import composed_task, lightsout_board, lightsout_logic
...
class LightsOutJacoEnv(WrappedDiscreteEnv):
    def __init__(
        self,
        reset_split,
        max_solution_depth,
        ...
        stairs: bool
            If True, the LightsOutBoard has elevated 'stairs'
        """
        manipulator = composed_task.JacoManipulator(PROPRIO_FEATURES)
        self.board = lightsout_board.LightsOutBoard(board_size, stairs=stairs)

        self._discrete_env = LightsOutDiscreteEnv(
        ...
        game_logic = lightsout_logic.LightsOutGameLogic(self._discrete_env)
    ...
    def plot_board(self, ax, board_state=None, initial_board_state=None):
        board = LightsOutTouchBoard(self.board_size)
    ...
    def plot_skill_episodes(
        ...
        n_rows, n_cols = self.board_size, self.board_size
        ...
        for row_idx, col_idx in product(range(n_rows), range(n_cols)):
            ...
            filtered_episodes = [
                ...
                if (e[0]["symbolic_action"] == symbolic_action)
