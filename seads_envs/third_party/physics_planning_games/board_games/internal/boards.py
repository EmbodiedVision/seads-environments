...
def _make_checkerboard(
    rows, columns, square_halfwidth, height=0.01, sensor_size=0.7, name="checkerboard"
):
    ...
    for i in range(rows):
        for j in range(columns):
            ...
            site = root.worldbody.add(
                "site", pos=(xpos[j], ypos[i], 2 * height), name=name
            )
...
def _make_goboard(
    boardsize, square_halfwidth, height=0.01, sensor_size=0.7, name="goboard"
):
    ...
    root = mjcf.RootElement(model=name)
    if _SHOW_DEBUG_GRID:
        black_mat = root.asset.add("material", name="black", rgba=(0.2, 0.2, 0.2, 0.5))
        white_mat = root.asset.add("material", name="white", rgba=(0.8, 0.8, 0.8, 0.5))
    else:
        transparent_mat = root.asset.add(
            "material", name="intersection", rgba=(0, 1, 0, 0.0)
        )
    ...
    root.asset.add(
        "texture", name="goboard", type="2d", file=mjcf.Asset(contents, ".png")
    )
    board_mat = root.asset.add(
        "material", name="goboard", texture="goboard", texrepeat=[0.97, 0.97]
    )
...
class CheckerBoard(composer.Entity):
    ...
    def validate_finger_touch(self, physics, row, col, hand):
        ...
        for contact in contacts:
            finger_tile_contact = (contact.geom1 in set1 and contact.geom2 in set2) or (
    ...
    def sample_pos_inside_touch_sensor(self, physics, random_state, row, col):
        ...
        jitter = bound_site.size * np.array([1.0, 1.0, 0.0])
...
class GoBoard(CheckerBoard):
    ...
    def sample_pos_inside_touch_sensor(self, physics, random_state, row, col):
        bound_site = physics.bind(self._touch_sensors[row, col].site)
        jitter = bound_site.size * np.array([0.25, 0.25, 0.0])
        return bound_site.xpos + random_state.uniform(-jitter, jitter)
