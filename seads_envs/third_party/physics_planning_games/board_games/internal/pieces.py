...
_RED = (1.0, 0.0, 0.0, 0.5)
_BLUE = (0.0, 0, 1.0, 0.5)

_INVALID_PLAYER_ID = "`player_id` must be between 0 and {}, got {}."
_NO_MORE_MARKERS_AVAILABLE = "All {} markers for player {} have already been placed."
...
class Markers(composer.Entity):
        ...
        for i, color in enumerate(player_colors):
            ...
            for j in range(num_per_player):
                player_markers.append(
                    root.worldbody.add(
                        "site", name="player_{}_move_{}".format(i, j), material=material
                    )
                )
    ...
    def _reset(self, physics):
        for player_markers in self._all_markers:
            for marker in player_markers:
                ...
                bound_marker.pos = 0.0  # Markers are initially placed at the origin.
                bound_marker.group = _INVISIBLE_SITE_GROUP
        self._move_counts = [0] * self._num_players
        self._marker_ids = np.zeros(
            (2, self._board_size, self._board_size), dtype=np.int32
        )
    ...
    def mark(self, physics, player_id, pos, bpos=None):
        ...
        if move_count >= len(markers):
            raise RuntimeError(_NO_MORE_MARKERS_AVAILABLE.format(move_count, player_id))
